...
band_list=('LST', 'TCWV', 'NDVI', 'biome', 'fraction', 'cloud_in', 
...
def write_query_file(query_file, date, footprint):
    ...
    beginPosition = '[%sT00:00:00.000Z TO %sT23:59:59.999Z]'%(date,date)
...
if __name__=='__main__':
    ...
    for s2_file in file_list:
        ...
        date, input_epsg, extent = get_sentinel2_date_extent(s2_file)
        footprint = sentinel_hub_footprint(extent, input_epsg)
        print(date)
        write_query_file(query_file, date, footprint)
        ...
        for input_file in download_list:
            ...
            if pth.exists(output_file+'.data'):
                sen.delete_DIMAP_bands(output_file, band_list)
